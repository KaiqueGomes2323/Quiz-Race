...
btnEntrar.addEventListener('click', async () => {
  ...
  try{
    ...
    if(room.mode === 'dinamico'){
      const maxPorGrupo = room.maxPorGrupo || MAX_JOGADORES_POR_GRUPO_PADRAO;
      const playerId = gerarIdJogador();
      let teamIdEscolhido = null;

      const txResult = await db.ref(`rooms/${roomCode}/teams`).transaction(teamsAtuais => {
        teamsAtuais = teamsAtuais || {};
        const ids = Object.keys(teamsAtuais);

        let menorQtd = Infinity;
        let escolhido = null;
        ids.forEach(tid => {
          const qtd = Object.keys(teamsAtuais[tid].players || {}).length;
          if(qtd < maxPorGrupo && qtd < menorQtd){ menorQtd = qtd; escolhido = tid; }
        });

        if(!escolhido){
          escolhido = 't' + ids.length;
          teamsAtuais[escolhido] = criarGrupoDinamico(ids.length);
        }

        teamsAtuais[escolhido].players = teamsAtuais[escolhido].players || {};
        teamsAtuais[escolhido].players[playerId] = playerName;
        teamIdEscolhido = escolhido;
        return teamsAtuais;
      });

      if(!txResult.committed || !teamIdEscolhido){
        mostrarErro('Não foi possível entrar na sala. Tente novamente.');
        btnEntrar.disabled = false;
        btnEntrar.textContent = 'Entrar na sala →';
        return;
      }

      await db.ref(`rooms/${roomCode}/players/${playerId}`).set({ name: playerName, teamId: teamIdEscolhido });

      salvarSessaoJogador(roomCode, playerId, playerName, teamIdEscolhido);
      window.location.href = `player.html?room=${roomCode}`;
      return;
    }
    ...
    teamIds.forEach(tid => {
      ...
      if(qtd < maxPorGrupo && qtd < menorQtd){ menorQtd = qtd; teamId = tid; }
    });

    if(!teamId){
      mostrarErro(`Todos os grupos já estão cheios (máximo de ${maxPorGrupo} pessoas por grupo).`);
      btnEntrar.disabled = false;
      btnEntrar.textContent = 'Entrar na sala →';
      return;
    }
