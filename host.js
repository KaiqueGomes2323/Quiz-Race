...
async function render(){
  if(currentRoom.status === 'lobby'){
    ...
    } else if(currentRoom.mode === 'dinamico'){
      document.getElementById('painelGrupos').classList.add('hidden');
      document.getElementById('painelIndividual').classList.add('hidden');
      document.getElementById('painelDinamico').classList.remove('hidden');
      renderTeamsDinamico();
    } else {
...
function renderTeams(){
  ...
  ids.forEach((tid) => {
    ...
    el.innerHTML = `
      ...
      <div class="grow"><strong>${escapeHtml(t.name)}</strong><span>${qtd}/${maxPorGrupo} jogador${qtd===1?'':'es'}</span></div>
    `;
    box.appendChild(el);
  });
}
...
function renderTeamsDinamico(){
  const teams = currentRoom.teams || {};
  const box = document.getElementById('teamListDinamico');
  const ids = Object.keys(teams).sort((a,b) => parseInt(a.slice(1),10) - parseInt(b.slice(1),10));

  const maxPorGrupo = currentRoom.maxPorGrupo || MAX_JOGADORES_POR_GRUPO_PADRAO;
  const maxInput = document.getElementById('maxPorGrupoDinamicoInput');
  if(document.activeElement !== maxInput) maxInput.value = maxPorGrupo;

  box.innerHTML = ids.length ? '' : '<p style="font-size:13px;">Ninguém entrou ainda, compartilhe o código da sala, o primeiro grupo é criado automaticamente.</p>';
  ids.forEach((tid) => {
    const t = teams[tid];
    const qtd = Object.keys(t.players || {}).length;
    const el = document.createElement('div');
...
const inputMaxPorGrupoDinamico = document.getElementById('maxPorGrupoDinamicoInput');
inputMaxPorGrupoDinamico.addEventListener('change', async () => {
  const n = Math.min(MAX_JOGADORES_POR_GRUPO_LIMITE, Math.max(1, parseInt(inputMaxPorGrupoDinamico.value, 10) || MAX_JOGADORES_POR_GRUPO_PADRAO));
  inputMaxPorGrupoDinamico.value = n;
...
const numAlternativasInput = document.getElementById('numAlternativasInput');
const altManualControls = document.getElementById('altManualControls');
const btnAddAlternativa = document.getElementById('btnAddAlternativa');
const btnRemoverAlternativa = document.getElementById('btnRemoverAlternativa');
...
function renderOptionInputs(n, preservar){
  const valoresAtuais = Array.from(optionsContainer.children).map(row => row.querySelector('input:not([type="radio"])')?.value || '');
  const correctAtual = optionsContainer.querySelector('input[name="qCorrect"]:checked');
  const correctIdxAtual = correctAtual ? parseInt(correctAtual.value, 10) : 0;

  optionsContainer.innerHTML = '';
  for(let i = 0; i < n; i++){
    const row = document.createElement('div');
    row.className = 'opt-row';
    row.innerHTML = `<input type="radio" name="qCorrect" value="${i}"${i === correctIdxAtual ? ' checked' : ''}><input id="opt${i}" maxlength="120" placeholder="Alternativa ${indiceParaLetra(i)}">`;
    optionsContainer.appendChild(row);
    if(preservar && valoresAtuais[i]){
      row.querySelector('input:not([type="radio"])').value = valoresAtuais[i];
    }
  }
  if(!optionsContainer.querySelector('input[name="qCorrect"]:checked')){
    optionsContainer.querySelector('input[name="qCorrect"]').checked = true;
  }
}

renderOptionInputs(parseInt(numAlternativasInput.value, 10) || 4, false);

numAlternativasInput.addEventListener('change', () => {
  if(numAlternativasInput.value === 'ilimitado'){
    altManualControls.classList.remove('hidden');
    const atual = optionsContainer.children.length;
    if(atual < MIN_ALTERNATIVAS){ renderOptionInputs(MIN_ALTERNATIVAS, true); }
    return;
  }
  altManualControls.classList.add('hidden');
  const n = Math.max(MIN_ALTERNATIVAS, parseInt(numAlternativasInput.value, 10) || 4);
  renderOptionInputs(n, true);
});

btnAddAlternativa.addEventListener('click', () => {
  renderOptionInputs(optionsContainer.children.length + 1, true);
});

btnRemoverAlternativa.addEventListener('click', () => {
  const atual = optionsContainer.children.length;
  if(atual > MIN_ALTERNATIVAS){ renderOptionInputs(atual - 1, true); }
});
...
document.getElementById('btnAddQuestion').addEventListener('click', async () => {
  ...
  const opts = optionRows.map(row => row.querySelector('input:not([type="radio"])').value.trim());
  ...
  renderOptionInputs(opts.length, false);
});
